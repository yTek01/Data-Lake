...
Database = "Postgres"
postgres_url = "jdbc:postgresql://yb-tserver-n1:5433/Postgres"
...
for i in order_table.collect():
    cur.execute("""INSERT INTO "staging"."orders" (order_id, customer_id, order_date, product_id, unit_price, quantity, amount) VALUES (%s, %s, %s, %s, %s, %s, %s)""",
...
orders = spark.read.format("jdbc") \
                ...
                .option("dbtable", f"staging.{Orders_Fact_tb_name}")    \
...
reviews = spark.read \
                .format("jdbc") \
                .option("url", postgres_url) \
                .option("dbtable", f"staging.{Reviews_Fact_tb_name}") \
...
shipment_deliveries = spark.read \
                .format("jdbc") \
                .option("url", postgres_url) \
                .option("dbtable", f"staging.{Shipments_Fact_tb_name}") \
                .option("user", POSTGRES_USER)  \
                .option("password", "") \
                .option("driver", "org.postgresql.Driver")  \
                .load()
